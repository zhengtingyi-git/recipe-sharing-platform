...
export default {
  ...
  methods: {
	...
	if(this.ruleForm.pengrenfangfa) {
		this.ruleForm.pengrenfangfa = this.ruleForm.pengrenfangfa.replace(reg,'../../../recipe-sharing-platform/upload');
	}
        } else {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `foreign_recipe/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.foreign_recipeCrossAddOrUpdateFlag = false;
						 this.parent.search();
